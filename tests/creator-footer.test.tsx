...
it('shows DoSthRk attribution and reveals the supplied profile details on click', () => {
  render(<CreatorFooter />);

  const trigger = screen.getByRole('button', {
    name: '查看作者 DoSthRk 的个人信息',
  });
  expect(screen.getByText('MADE BY')).toBeInTheDocument();
  expect(screen.getByText('DoSthRk')).toBeInTheDocument();
  expect(trigger).toHaveAttribute('aria-expanded', 'false');
  expect(
    screen.queryByRole('region', { name: '作者 DoSthRk 的个人信息' }),
  ...
  expect(
    screen.getByRole('img', { name: 'DoSthRk 的头像' }),
  ...
  expect(
    screen.getByRole('img', { name: 'DoSthRk 的微信二维码' }),
