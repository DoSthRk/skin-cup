import type { RawSkin, Skin, WeaponConfig, WeaponId } from './types';

const weaponIds: Record<string, WeaponId> = {
  狂徒: 'vandal',
  幻影: 'phantom',
  正义: 'sheriff',
};

const allowedEffectItems = new Set([
  'VFX',
  'Animation',
  'Finisher',
  'SoundEffects',
...
const approvedSpecialSkinNames = new Set([
  '2025全球冠军赛 狂徒',
  '海洋之星 狂徒',
  '侦察力量 幻影',
  '猩红猛兽 正义',
...
const rejectedSkinNames = new Set([
  '黑市 狂徒',
  '猩红猛兽 狂徒',
  '灵魂冲击 幻影',
  '涂鸦伙伴 幻影',
...
function effectName(levelItem: string): string {
  const parts = levelItem.split('::');
  return parts[parts.length - 1] ?? levelItem;
}

function approvedEffects(levels: readonly RawSkin['levels'][number][]): string[] {
  return [
    ...new Set(
      levels
        .map((level) => level.levelItem)
        .filter((levelItem): levelItem is string => typeof levelItem === 'string')
        .map(effectName)
...
export function filterCatalog(rawSkins: readonly RawSkin[]): Skin[] {
  const catalog = new Map<string, Skin>();
